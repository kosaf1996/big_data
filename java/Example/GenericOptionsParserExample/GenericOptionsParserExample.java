...
public class GenericOptionsParserExample {
    ...
    public static void main(String[] args) throws IOException { 
        System.out.println(Arrays.toString(args)); //args print 

        Configuration conf = new Configuration(); //conf 객체 
        // conf안에 arg로 들어온 부분들을 셋팅해준다

        GenericOptionsParser optionsParser = new GenericOptionsParser(conf, args); //optionsParser 객체에 conf 와 args 전달 

        String value1 = conf.get("mapreduce.map.memory.mb"); //mapreduce.map.memory.mb (memory 용량 설정)설정
